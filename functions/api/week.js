...
export async function onRequestGet({ env, request }) {
  ...
  const grid = habits.map(h => {
    const scheduled = h.days ? (() => { try { return JSON.parse(h.days) } catch { return null } })() : null
    return {
      ...h,
      scheduledDays: scheduled,
      days: weekDates.map((d, i) => {
        const dayNum = i + 1  // weekDates[0] is Monday = 1
        const isScheduled = !scheduled || scheduled.includes(dayNum)
        return { ...(logs.find(l => l.habit_id === h.id && l.date === d) ?? { date: d, done: 0 }), scheduled: isScheduled }
      }),
    }
  })

  return Response.json({ days: weekDates, habits: grid })
}
